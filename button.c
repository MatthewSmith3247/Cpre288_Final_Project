...
void init_button_interrupts() {
    ...
    // 5) Un-mask the bits for pins 0-3
...
uint8_t button_getButton() {
	...
	//
	// DELETE ME - How bit masking works
	// ----------------------------------------
    ...
    if(~GPIO_PORTE_DATA_R & 0b00001000){ //PB4 , need to ~ due to buttons as active lows
        pb = 4;

    } else if(~GPIO_PORTE_DATA_R & 0b00000100){ //{PB3
        pb = 3;

    } else if(~GPIO_PORTE_DATA_R & 0b00000010){ //{PB2
        pb = 2;

    } else if(~GPIO_PORTE_DATA_R & 0b00000001){ //{PB1
        pb = 1;
    }
